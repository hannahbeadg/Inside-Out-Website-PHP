...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($_FILES['emotion_image']['name'])) {

            $new_image = $_FILES['emotion_image']['name'];
            $tmp_name = $_FILES['emotion_image']['tmp_name'];

            move_uploaded_file($tmp_name, "uploads/" . $new_image);

            mysqli_query(
                $conn,
                "UPDATE tbl_emotions SET
                    emotion_name='$emotion_name',
                    emotion_image='$new_image',
        ...
        $_SESSION['status'] = "Emotion Successfully Updated!";
        header("location: adminemotions.php");
        exit();
    }

    $query = mysqli_query($conn, "SELECT * FROM tbl_emotions WHERE emotion_id='$emotion_id'");
